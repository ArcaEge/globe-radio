 ...
 *                  Macros, Variables
 *======================================================*/
#define MP3_AUDIO_BUF_SZ    (4 * 1024)  /* input buffer size */
 ...
 *          Private functions
 *======================================================*/
static int readFromStr(void* stream, char* buffer, uint count) {
...
static int32_t mp3_decoder_fill_buffer(struct mp3_decoder *decoder) {
    ...
    bytes_read = readFromStr(decoder->fetch_parameter, decoder->read_buffer + decoder->bytes_left, bytes_to_read);
...
int mp3_decoder_run(struct mp3_decoder *decoder) {
    ...
    if ((len = mp3_decoder_run_internal(decoder, buffer)) > 0) {
        ...
        printf("mp3_decoder_run_internal failed\n");
        return len;
    }
}